    ...
    CredentialsProvider({
      ...
      async authorize(credentials) {
        try {
          ...
          if (!user || !role) {
            throw new Error(
              "The Provided credentials does not match with our records.",
            );
          }
  ...
  callbacks: {
    async jwt({ token, user }) {
      if (user) {
        ...
        token.hasCompletedPlanSelection =
          (user as any).hasCompletedPlanSelection ?? false;
      }
      return token;
    },
    async session({ session, token }) {
      if (session.user) {
        ...
        (session.user as any).hasCompletedPlanSelection =
          token.hasCompletedPlanSelection ?? false;
      }
      return session;
    },
  },
