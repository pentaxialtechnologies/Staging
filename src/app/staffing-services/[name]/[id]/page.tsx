    ...
    const fetchData = async () => {
      try {
        const res = await fetch(
          `/api/auth/organization/staff/by-org?orgId=${id}`,
        );
  ...
  return (
    <div className="w-full min-h-screen bg-gray-50 py-10 px-4 sm:px-6 lg:px-10">
      ...
      <p className="text-center text-gray-600 mb-8">
        Hire Ready-To-Deploy Technology Talents From Thousands Of Pre-Vetted
      ...
      {loading ? (
        ...
        <p className="text-center text-gray-500">
          No approved staff found for this organization.
        </p>
      ) : (
        <div className="flex flex-col gap-10">
          {StaffData.map((Staff) => (
            <div
              key={Staff._id}
              className="w-full bg-white p-6 rounded-md shadow border border-gray-200"
            >
              {/* Company Info */}
              <div className="flex flex-col sm:flex-row items-center gap-6 mb-6">
                ...
                <div className="text-center sm:text-left">
                  <h2 className="text-2xl font-semibold text-gray-800">
                    {Staff.OrgId.company_name}
                  </h2>
                  <p className="text-gray-500">
                    {Staff.OrgId.location.city}, {Staff.OrgId.location.state}
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 text-gray-700 text-base">
                <div>
                  <p>
                    <strong>Primary Skills:</strong> {Staff.primarySkills}
                  </p>
                  <p>
                  ...
                  <p>
                    <strong>Rate Type:</strong> {Staff.rateType}
                  </p>
                  <p>
                    <strong>Experience:</strong> {Staff.averageExperience} years
                  </p>
                </div>
                <div>
                  <p>
                ...
                <div>
                  <button className=" text-white bg-[#f27264] px-4 py-2 rounded-full">
