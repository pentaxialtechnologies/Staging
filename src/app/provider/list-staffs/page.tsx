    ...
    const fetchStaff = async () => {
      try {
        const res = await fetch(
          `/api/auth/organization/staff-list?orgId=${orgId}`,
        );
  ...
  const handleDeleteStaff = async (id: string) => {
    try {
      const res = await axios.delete(
        `/api/auth/organization/staff/delete/${id}`,
      );

      if (res.status === 200) {
        alert("Staff Deleted Successfully!");
        setStaffList((prevList) =>
          prevList.filter((staff) => staff._id !== id),
        );
      } else {
    ...
    <div className="p-6">
      ...
      <div className="w-full flex justify-end mb-4">
        <button
          ...
          className="bg-[#f27264] text-white font-bold px-6  py-2 rounded-full shadow-md hover:shadow-xl hover:bg-[#f27264] "
        >
          Add Staff
        </button>
      </div>
        ...
        <div className="flex flex-row gap-6">
          ...
          <div>
            ...
            <p className="text-base tracking-normal">
              {companyData.company_location},{companyData?.location?.city},
        ...
        <div className="bg-white shadow-md p-6 rounded-lg">
          <div className="grid grid-cols-1 gap-6">
            {staffList.map((staff, index) => (
              <div
                key={index}
                className="bg-white shadow-lg border rounded-xl p-6 transition hover:shadow-xl"
              >
                {/* Top Controls */}
              ...
              <div className="absolute left-1 top-1 w-4 h-4 bg-white rounded-full transition peer-checked:translate-x-5" />
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 text-sm">
                  <div className="space-y-1">
                    ...
                    <p>
                      <strong>Number Bench Staff:</strong>{" "}
                      {staff.numberBenchStaff}
                    </p>
                    <p>
                      <strong>Rate:</strong> ${staff.rate} (Hourly)
                    </p>
                    <p>
                      <strong>Experience:</strong> {staff.averageExperience}
                    </p>
                    <div className="flex flex-wrap gap-2 mt-2">
                      ...
                      {staff.skills.map((skill, idx) => (
                        <span
                          key={idx}
                          className="bg-blue-200 text-blue-800 px-2 py-1 text-xs rounded-full"
                        >
                          {skill}
                        </span>
                      ))}
                  ...
                  <div className="space-y-1">
                    <p>
                      <strong>Work From:</strong> {staff.workFrom}
                    </p>
                    <p>
                      <strong>Engagement Type:</strong> {staff.engagementType}
                    </p>
                    <p>
                      <strong>Available at Client Location:</strong>{" "}
                    ...
                    <p className="font-semibold">
                      Active Status:{" "}
                      <span
                        className={`ml-1 px-2 py-1 rounded-full text-xs ${staff.isApproved ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}
                      >
