...
const ProfilePage = () => {
  ...
  const removeProjectLink = (projectIndex: number, linkIndex: number) => {
    setFormData((prev) => {
      ...
      updatedPortfolio[projectIndex] = {
        ...updatedPortfolio[projectIndex],
        project_link: updatedPortfolio[projectIndex].project_link.filter(
          (_, i) => i !== linkIndex,
        ),
      };
      return { ...prev, portfolio: updatedPortfolio };
  ...
  const removeSkill = (skillremove: string) => {
    setFormData((prev) => ({
      ...prev,
      skills: prev.skills.filter((skill) => skill !== skillremove),
    }));
  };
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const res = await fetch(
          `/api/auth/organization/update/${session.user.id}`,
        );
  ...
  return (
    <div>
      <div className="flex flex-row justify-between">
        ...
        <button
          onClick={() => SwitchToListing()}
          className="mb-5 text-white bg-blue-800 px-4 py-2 rounded-lg"
        >
          Bact To Listing{" "}
        </button>
      </div>
      ...
      >
        ...
        {step === 3 && (
          <div className="flex flex-col">
            <h1 className="text-center py-5 text-3xl font-bold text-gray-700">
            ...
            {formdata.gst_status === "Yes" ? (
              <div className="mt-4">
                <label htmlFor="gstno" className="block mb-5 font-medium">
                  GSTIN - INDIA
                </label>
                <input
                  type="text"
                  ...
                  onChange={handleChange}
                />
                <p className="text-base text-justify tracking-normal px-4 py-2">
                  Your GSTIN should have 15 characters. It can start and end
                  with a letter or number, and should look like this:
                  23RSCCC6023W6Z8
                </p>
              </div>
            ) : (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-5">
              ...
              <div className="flex flex-col">
                ...
                {formdata.portfolio.length > 0 &&
                  Array.isArray(formdata.portfolio[0].project_link) && (
                    <>
                      {formdata.portfolio[0].project_link.map((link, index) => (
                        ...
                        >
                          <input
                            ...
                            value={link || ""}
                            onChange={(e) =>
                              handleProjectLinkChange(0, index, e.target.value)
                            }
                          />
