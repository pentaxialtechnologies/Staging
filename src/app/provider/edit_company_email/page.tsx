...
const Page = () => {
  ...
  const handleEmailChange = async () => {
    ...
    try {
      const send = await fetch("/api/auth/provider/change-email", {
        method: "POST",
        body: JSON.stringify({
          userId: userId,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const verify = await fetch(
        "/api/auth/provider/change-email/verify-code",
        {
          method: "POST",
          body: JSON.stringify({
            userId: userId,
  ...
  const handleResendCode = async () => {
    ...
    try {
      const resend = await fetch("/api/auth/provider/change-email", {
        method: "POST",
        body: JSON.stringify({
          userId: userId,
          email: FormData.email,
  ...
  return (
    <div>
      <div>
        ...
        <button
          onClick={() => router.push("/provider/setting")}
          className="bg-orange-600 text-white px-4 py-2 mt-4 w-20 rounded-full mb-5"
        >
          Back
        </button>
        <div>
          <form onSubmit={handleSubmit}>
            ...
            {codesent && (
              <div>
                <label className="block mb-2 text-xl mt-5">
                ...
                <input
                  ...
                  onChange={handleChange}
                  className="w-90 px-4 py-2 border border-gray-800"
                />
            ...
            {codesent && (
              <button
                type="submit"
                disabled={loading}
                className="bg-orange-700 px-4 py-2 text-white rounded-full mt-5"
              >
