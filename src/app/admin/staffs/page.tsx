      ...
      <div className="w-full max-x-xl mx-auto mb-6 mt-5">
        <div className="relative">
          ...
          <input
            ...
            placeholder="Search by designation or skill..."
            className="w-full px-4 py-2 pl-10 pr-4 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 "
          />
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col gap-6 p-6">
          {Paginated.map((staffs) => (
            <div
              key={staffs._id}
              className="bg-white p-6 border border-gray-150 shadow-md transition"
            >
              <div className="flex flex-row gap-8  mb-5 ">
                <div>
                  <Image
                    src={staffs.OrgId.companylogo}
                    alt="logo"
                    className="object-cover w-60 h-auto rounded"
                  />
                </div>
                <div>
                  <p className="text-xl sm:text-2xl">
                    {staffs.OrgId.company_name}
                  </p>
                </div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div className="text-sm text-gray-800 gap-5">
                  <p>
                    <strong>Primary Skills:</strong> {staffs.primarySkills}
                  </p>
                  <p>
                    <strong>Designation:</strong> {staffs.designation}
                  </p>
                  <p>
                    <strong>No. of Bench Staff:</strong>{" "}
                    {staffs.numberBenchStaff}
                  </p>
                  <p>
                    <strong>Rate Type:</strong> {staffs.rateType}
                  </p>
                  <p>
                    <strong>Experience:</strong> {staffs.averageExperience}{" "}
                    years
                  </p>
                </div>
                <div className="text-sm text-gray-800">
                  <p>
                    <strong>Work From:</strong> {staffs.workFrom}
                  </p>
                  <p>
                    <strong>Engagement Type:</strong> {staffs.engagementType}
                  </p>
                  <p>
                    <strong>Availability:</strong> {staffs.availability}
                  </p>
                  <p>
                    <strong>Client Location:</strong>{" "}
                    {staffs.availableAtClientLocation}
                  </p>
                  <p>
                    <strong>Active Date :</strong>{" "}
                    {new Date(staffs.updatedAt).toLocaleString("en-GB", {
                      day: "2-digit",
                      month: "long",
                      year: "numeric",
                    })}
                ...
                <div className="flex flex-row gap-6">
                  <button
                    onClick={() => handleStatusApprove(staffs._id, true)}
                    className="bg-green-200 rounded-full text-green-900 px-4 py-2"
                  >
                    Approve
                  </button>
                  <button
                    onClick={() => handleStatusHold(staffs._id, false)}
