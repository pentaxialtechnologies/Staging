      ...
      {data ? (
        <div className="bg-white shadow-xl rounded-2xl p-6 space-y-6 border border-gray-200">
                ...
                onChange={async (e) => {
                  ...
                  try {
                    const res = await fetch(
                      `/api/auth/jobs/${params?.id}/status`,
                      {
                        method: "PUT",
                        headers: {
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
            ...
            <p>
              <strong>Work Mode:</strong> {data.workmode}
            </p>
            <p>
              <strong>Currency:</strong> {data.currency_type}
            </p>
            <p>
              <strong>Experience:</strong> {data.experience.minyears} -{" "}
              {data.experience.maxyears} yrs
            </p>
            <p>
              <strong>Engagement Type:</strong> {data.engagement_type}
            </p>
            <p>
              <strong>Payment Schedule:</strong> {data.payment_schedule}
            </p>
            <p>
              <strong>Rate:</strong> {data.rate}
            </p>
            <p>
              <strong>Working Days:</strong> {data.working_days.start_day} -{" "}
              {data.working_days.end_day}
            </p>
            <p>
              <strong>Working Hours:</strong> {data.working_hours.start_time} -{" "}
              {data.working_hours.end_time}
            </p>
            <p>
              <strong>Start Date:</strong> {data.job_dates.start_date}
            </p>
          </div>
