    ...
    const FetchData = async () => {
      ...
      try {
        const res = await fetch(
          `/api/auth/contractjobs/get/${session.user.id}`,
        );
        if (!res.ok) {
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {datas.map((job: any) => (
            ...
            >
              <button
                onClick={() => router.push(`/employer/job-list/${job._id}`)}
                className="text-2xl mb-2 text-violet-600 rounded hover:text-blue-700"
              >
                {job.title}
              </button>
              ...
              <p>
                Experience: {job.experience.minyears} -{" "}
